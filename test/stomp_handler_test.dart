...
void main() {
  group('StompHandler', () {
    ...
    setUpAll(() async {
      ...
      port = await (streamChannel.stream.first as FutureOr<int?>);
      config = StompConfig(
    ...
    test('unsubscribes correctly', () {
      ...
      handler = StompHandler(
          config: config.copyWith(
              onConnect: (_, frame) {
                dynamic Function({Map<String, String> unsubscribeHeaders}) unsubscribe = handler!.subscribe(
                    destination: '/foo',
                    callback: onSubscriptionFrame,
                    headers: {'id': 'sub-0'});
                Timer(Duration(milliseconds: 500), () {
    ...
    test('correctly logs data not subtype of String', () {
      ...
      dynamic onDebugMessage = expectAsync1((dynamic mgs) {}, count: 1, max: -1);
      // We need this async waiter to make sure we actually wait until the
      // connection is closed to not affect other tests
      dynamic onDisconnect = expectAsync1((dynamic frame) {}, count: 1);
